...
def create_indexes_from_kmers(directory):
    ...
    for kmer in test_kmers:
        ...
        cobs.compact_construct(directory,"indexes_all/" + str(kmer) + "_index.cobs_compact", index_params=params)
    return test_kmers
...
os.mkdir("files_all") 
os.mkdir("indexes_all") 
os.mkdir("results_all") 

all_index_list = []
for i in range(len(cleaned)):
    all_index_list.append(i)
    
proteins = []
gene_names = []

print("Separating proteins...")
for protein in all_index_list:
    info = cleaned[protein]
    proteins.append(info)
    gene_names.append(cluster_name["gene"][protein])

filename_list = [[all_index_list]]
...
for file in filename_list:
    ...
    for a in range(len(file)):
        for c in tqdm(range(len(file[a]))):
            temp_dir = os.path.join(tempfile.mkdtemp(dir="files_all"), "") 
            without_y = file[a][:]
            del without_y[c]
            ...
            kmer_df.to_csv("results_all/" + str(count) + "_" + str(len(file[0])) + ".csv")
            count += 1
